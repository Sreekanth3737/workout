...
console.log(centuryFromYear(2024));
...
function coolString(inputString) {
  ...
  for (var i = 0; i < inputString.length - 1; i++) {
    ...
    if ((isLower(curr) && isLower(next)) || (isUpper(curr) && isUpper(next)))
      return false;
  }
  return true;
}
...
function areSimilar(A, B) {
  ...
  return (
    falseA.sort((a, b) => a - b).join("") ===
    falseB.sort((a, b) => a - b).join("")
  );
}
...
function electionsWinners(votes, k) {
  ...
  return k === 0
    ? votes.indexOf(max) === votes.lastIndexOf(max)
      ? 1
      : 0
    : count;
}
...
function arrayMedian(sequence) {
  ...
  if (sequence.length % 2 === 0) {
    return (
      (sequence[sequence.length / 2] + sequence[sequence.length / 2 - 1]) / 2
    );
  }
...
function variableName(name) {
  return name.match(/^[a-zA-Z_]+[a-zA-Z0-9_]*/) === null
    ? false
    : name.match(/^[a-zA-Z_]+[a-zA-Z0-9_]*/)[0].length === name.length;
}
...
function isPermutation(n, inputArray) {
  return (
    inputArray.sort((a, b) => a - b).filter((item, idx) => item === idx + 1)
      .length === n
  );
}
